...
let ChartWizard = React.createClass({
  ...
  render () {
    let countries = MenuItem.fromArray(this.state.data.countryList, ChartWizardActions.selectCountry)
    let countryStep = (
      <div>
    ...
    let subLocations = MenuItem.fromArray(filterMenu(this.state.data.subLocationList, this.state.locationSearch), ChartWizardActions.addLocation)
    let locationStep = (
      <div>
    ...
    let firstIndicatorStep = (
      <div>
        ...
        <span className='chart-wizard__next' onClick={this.toggleStep.bind(null, 'sub-location')}>Next</span>
      </div>
    )
    ...
    return (
      <div className='chart-wizard'>
        <ChartWizardStepList onToggle={this.toggleStep} active={this.state.refer}>
          <ChartWizardStep title={`1. Select Country - ${this.state.data.location && this.state.data.location.name}`}
            refer='location'>
            {countryStep}
          </ChartWizardStep>
          ...
          <ChartWizardStep title='3. Select Location' refer='sub-location'>
            {locationStep}
          </ChartWizardStep>
